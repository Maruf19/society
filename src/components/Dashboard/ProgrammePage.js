...
const ProgrammePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let url = editingId
        ? `http://localhost:5000/programme/${editingId}`
        : 'http://localhost:5000/programme';
 ...
 const handleDelete = async (id) => {
  if (window.confirm('Are you sure you want to delete this item?')) {
    try {
      const response = await fetch(`http://localhost:5000/programme/${id}`, {
        method: 'DELETE',
      });
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div id="form-container" className="bg-white border border-black rounded-lg shadow-lg px-8 py-2 w-96 max-w-full">
          <h1 className="text-2xl font-semibold text-center mb-2">
            {editingId ? 'Edit Programmw' : 'Add Programme'}
          </h1>
