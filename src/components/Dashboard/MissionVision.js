...
const MissionVisionPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingId ? `http://localhost:5000/mission-vision/${editingId}` : 'http://localhost:5000/mission-vision';
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:5000/mission-vision');
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        const response = await fetch(`http://localhost:5000/mission-vision/${id}`, {
          method: 'DELETE',
        });
