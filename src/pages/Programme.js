  ...
  return (
    <>
      ...
      <section className="container mx-auto py-6 px-4 md:px-6 lg:px-8 flex flex-col items-center">
        <h2 className="text-4xl font-bold text-center text-gray-800 mb-6 border-b-2 border-blue-500 pb-2 inline-block">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10 lg:gap-12 w-full">
          {programmeData.map(({ id, title, imageUrl, description }) => (
            <motion.div
              key={id}
              className="bg-white shadow-lg rounded-lg overflow-hidden w-full max-w-sm mx-auto flex flex-col border border-blue-500"
              initial="hidden"
              animate="visible"
            ...
            >
              <div className="relative">
                <img
                  className="w-full h-48 md:h-56 lg:h-64 object-cover border border-blue-500"
              ...
              <div className="p-4 md:p-6 flex flex-col flex-1">
                <h2 className="text-lg md:text-xl font-bold text-blue-500 mb-4">{title}</h2>
                <p className="text-gray-600 mb-6 flex-1 text-sm md:text-base">{description}</p>
                <div className="flex flex-col sm:flex-row sm:space-x-4 mt-auto justify-center">
                  <motion.button
                    className="px-4 py-2 mb-2 sm:mb-0 border border-blue-500 text-blue-500 rounded-full bg-transparent hover:bg-blue-500 hover:text-white transition-all duration-300"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                  >
                    Learn More
                  ...
                  <Link to="/review">
                    <motion.button
                      className="px-4 py-2 border border-blue-500 text-blue-500 rounded-full bg-transparent hover:bg-blue-500 hover:text-white transition-all duration-300"
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                    >
                      Review
